...
<img src="https://storage.googleapis.com/cline_public_images/docs/assets/workflows.png" alt="Workflows tab in Kilo Code" />
...
You have access to the `gh` terminal command. I already authenticated it for you. Please review it to use the PR that I asked you to review. You're already in the `kilocode` repo.
...
3. Let Kilo Code handle everything else

My workflow uses the `gh` command-line tool and Kilo Code's built in `ask_followup_question` to:
...
-   If I say "yes," Kilo Code automatically approves the PR with the `gh` command
...
> This is just one example of a workflow file. You can find more in our [prompts repository](https://github.com/Kilo-Org/kilocode/.kilocode/workflows) for inspiration.
