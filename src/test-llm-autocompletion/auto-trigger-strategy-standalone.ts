import { UseCaseType } from "./types.js"

const CURSOR_MARKER = "<<<AUTOCOMPLETE_HERE>>>"

/**
 * Standalone version of AutoTriggerStrategy for testing
...
## Response Format
Return suggestions as XML:
<suggestion>
  <change>
    <search><![CDATA[text_with_cursor_marker]]></search>
    <replace><![CDATA[completed_text]]></replace>
  </change>
</suggestion>

If no suggestion, return empty: <suggestion></suggestion>`
	}

	/**
	 * Build prompt for auto-trigger based on context
	 */
	getUserPrompt(context: any): string {
		...
		if (context.recentOperations && context.recentOperations.length > 0) {
			...
			context.recentOperations.forEach((op: any, index: number) => {
		...
		if (document) {
			prompt += "## Full Code\n"
			prompt += this.formatDocumentWithCursor(document, range)
			prompt += "\n\n"
		}
	...
	/**
	 * Format document with cursor marker
	 */
	private formatDocumentWithCursor(document: any, range: any): string {
		if (!range) return document.getText()

		const text = document.getText()
		const offset = document.offsetAt(range.start)

		// Insert cursor marker at the offset
		return text.slice(0, offset) + CURSOR_MARKER + text.slice(offset)
	}
