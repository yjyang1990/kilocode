...
export async function loadContext({
	...
	let needsKilorulesFileCheck = false
	/**
			...
			userContent.map(async (block) => {
				...
				if (block.type === "text") {
					if (shouldProcessMentions(block.text)) {
						...
						const { processedText, needsKilorulesFileCheck: needsCheck } = parseSlashCommands(parsedText)

						if (needsCheck) {
							needsKilorulesFileCheck = true
						}
