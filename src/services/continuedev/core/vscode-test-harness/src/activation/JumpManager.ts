...
const SVG_CONFIG = {
	...
	getFontSize(): number {
		return Math.ceil((vscode.workspace.getConfiguration("editor").get<number>("fontSize") ?? 14) * 0.8)
	},
...
export class JumpManager {
  ...
  <text x="4" y="${textY}" font-family="${fontFamily}" font-size="${fontSize}" fill="${SVG_CONFIG.stroke}">${SVG_CONFIG.label}</text>
	...
	private async registerKeyListeners(editor: vscode.TextEditor, jumpPosition: vscode.Position) {
