...
import { type ZAIModelId, zaiDefaultModelId, zaiModels } from "@roo-code/types"
...
const ZAI_DEFAULT_TEMPERATURE = 0
...
export class ZAIHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		this.client = new OpenAI({
			...
			apiKey: this.options.zaiApiKey ?? "not-provided",
	...
	override getModel() {
		const id =
			this.options.apiModelId && this.options.apiModelId in zaiModels
				? (this.options.apiModelId as ZAIModelId)
				: zaiDefaultModelId

		const info = zaiModels[id]
	...
	override async *createMessage(
		...
		const stream = await this.client.chat.completions.create({
			...
			temperature: this.options.modelTemperature ?? ZAI_DEFAULT_TEMPERATURE,
	...
	async completePrompt(prompt: string): Promise<string> {
		...
		try {
			...
			if (error instanceof Error) {
				throw new Error(`zAI completion error: ${error.message}`)
			}
